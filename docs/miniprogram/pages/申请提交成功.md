...
1. **进入方式仅两种合法路径**  
   - [申请成为英雄](./申请成为英雄.md) 提交成功 → `wx.redirectTo` 本页（**必须 replace 表单页**）  
   - [个人中心](./个人中心.md) 审核中 → 点击「查看审核进度」→ `wx.navigateTo` 本页  

2. **返回行为**  
   - 导航栏 ‹、底部按钮、系统返回键：**统一** `wx.switchTab({ url: '/pages/profile/profile' })`  
   - **禁止**回到申请表单（历史栈中不应存在 `hero-apply`）  
   - 预览 SPA：`data-back-target="profile.html"` 固定回 Tab「我的」

3. **通知文案**  
   - M1：静态文案「审核通过后会发送通知」；M2 接订阅消息/消息中心 [消息](./消息.md)

4. **重复进入**  
   - 审核中用户可多次从个人中心进入本页查看说明，**不重复创建申请**
