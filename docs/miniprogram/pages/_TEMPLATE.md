...
> **铁律**：只根据原型改动写入本文；不要按本文去改原型。AI 补充时不覆盖你的原文；`【重复】`（紫）/ `【矛盾】`（红）为待确认标记。  
> **表格序号铁律**：单元格内分点必须写 `<br>`，例如 `1、…<br>2、…`（见 `.cursor/rules/doc-table-linebreaks.mdc`）。

## UI说明

（给设计师的提示，不写颜色/圆角等样式句；预览会提到元信息下方并用紫色突出。）
...
表格单元格若有多条序号分点，**必须**用 `<br>` 换行：

```markdown
| 展示内容 | 说明 |
|----------|------|
| 示例 | 1、第一点<br>2、第二点<br>3、第三点 |
```
