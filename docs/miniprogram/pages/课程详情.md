...
带 id 进入 → 展示课程 → 一键报名写 `signups`（`type=course`）→ 可 `signed+1`。

### 2.4 核心业务规则

1. 课程不存在 → Toast「课程不存在」
2. 报名写入 `signups`，`type=course`，关联 `course_id`
3. 报名成功课程 `signed+1`（以实现为准）
4. 列表/详情数据与发布英雄 `hero_id` 关联
5. M1 无表单，固定 Mock 姓名/手机

### 2.5 异常与边界

- Banner / 详情字段缺失时用默认图或纯文本兜底
