...
1. `onLoad` 读取 `statusBarHeight` 适配 Banner 安全区，并 `loadData()` 填充 Mock
2. Banner CTA「查看活动」跳转 **第一条** `events[0]` 的招募详情；无活动时无跳转
3. 区块标题「更多」、精选活动卡片点击 → `switchTab` 英雄广场 或 `navigateTo` 招募详情
4. 英雄卡片点击 → `navigateTo` 英雄详情 `?id=hero_id`
5. 精选课程卡片、好物卡片、新闻条目 **M1 无点击事件**（仅展示）
6. 支持下拉刷新，重新 `loadData()` 后 `stopPullDownRefresh`
