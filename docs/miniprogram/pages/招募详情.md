...
1. `id` 缺失 → Toast **活动不存在**
2. 并行 `getRecruitmentById` + `getMySignupByRecruitId` 初始化 footer 状态
3. `onPageScroll`：scrollTop ≥ coverThreshold(180) → navSolid 显示标题
4. 封面：多图 swiper 2s 自动轮播；单图占位
5. 报名成功：`signed+1`、关闭表单、刷新 footer、Toast **报名成功**
6. 核销成功：signup checked_in、Toast **核销成功**
7. 表单校验：姓名 trim 非空；手机 `/^1\d{10}$/`
