...
1. `onShow` → `loadProfile()`：并行拉取 `getHeroApplyStatus`、`getMySignupSummary`、`getMyReviewCount`
2. `isHero=true` 时展示英雄统计与英雄中心；`heroPending=true` 时 CTA 文案与跳转不同
3. 点击「已认证英雄 ›」→ [认证成功](./认证成功.md)（仅 isHero）
4. 「发布招募/课程」→ 底部 ActionSheet：发布赛事招募 / 发布课程 / 取消
5. 英雄中心各入口需 `isHero` 守卫，非英雄不响应
6. 开发开关 `onToggleHeroState` 在 approved ↔ none 间切换并写 storage
