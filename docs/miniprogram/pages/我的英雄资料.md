...
1. `onLoad/onShow` → `loadHero()` via `data.getHeroById('1')`
2. 关于我「编辑」→ storage 当前文案 → navigateTo bio-edit?from=hero-profile&field=about
3. 荣誉：新增/编辑 sheet；名称必填；删除需 Modal 确认
4. 精彩瞬间/证书：最多 **9** 张；chooseMedia album；长按拖动排序（vibrateShort）
5. 删除图片 × 即时 persist；预览用 image-viewer
6. 添加证书默认名 **证书{n}**
