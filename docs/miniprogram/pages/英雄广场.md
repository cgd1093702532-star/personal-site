...
进入列表 → API 优先拉英雄（mock 兜底）→ 搜索/筛选 → 进英雄详情。

### 2.4 核心业务规则

1. 列表 API 优先，mock 兜底
2. 后台新批准英雄应出现在列表
3. 搜索 300ms 防抖；筛选：项目 / 排序 / 从业年限；非默认计入 filterCount
4. 筛选 chip 即时生效；「完成」仅关面板；「重置」恢复默认
5. 商城区块若有则仅入口占位（本页以英雄列表为主）
6. `onShow` 重新拉列表

### 2.5 异常与边界

- keyword 无结果 / 无 keyword 无结果空态文案不同

### 2.6 待确认项

- [ ] 筛选/排序产品定稿细节（保留现有 chip 规则，并追加「API 优先」）
